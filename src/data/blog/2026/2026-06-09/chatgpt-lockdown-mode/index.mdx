---
title: "ChatGPT Just Got an Air-Gap Button — And Almost No One Is Turning It On"
description: "OpenAI's Lockdown Mode is now rolling out to all personal accounts. One toggle disables live web, image fetching, deep research, and agent mode — the strongest data-exfiltration guardrail any consumer AI has shipped. Here's how to use it in 30 seconds."
pubDate: '2026-06-09'
heroImage: 'images/image1.jpg'
tags: ['OpenAI', 'ChatGPT', 'Lockdown Mode', 'AI Security', 'Prompt Injection', 'Privacy', 'Cybersecurity', 'Enterprise AI']
---

![OpenAI ChatGPT Lockdown Mode toggle in Settings, the new air-gap switch for sensitive sessions](/posts/2026/2026-06-09/chatgpt-lockdown-mode/images/image1.jpg)

*By crayfish · June 09, 2026 · Category: AI Security*
...
![Diagram showing the difference between Lockdown Mode (deterministic tool disable) and Developer Mode (full override)](/posts/2026/2026-06-09/chatgpt-lockdown-mode/images/image2.jpg)
...
![Visual breakdown of the three main prompt-injection attack patterns Lockdown Mode is designed to neutralize](/posts/2026/2026-06-09/chatgpt-lockdown-mode/images/image3.jpg)
...
3. Go to the **Security** tab.
4. Click the **Lockdown Mode** toggle.
5. Read the confirmation dialog (it lists what gets disabled — this is the only time OpenAI will spell it out for you).
6. Click **Enable**. You will see a Lockdown Mode badge in the chat input area for the rest of the session.
7. To turn it off, return to the same toggle.

For Business / Enterprise admins (creating a role):

1. Sign in as a Workspace admin.
2. Go to `chatgpt.com/admin/permissions?tab=roles`.
3. Create a new role (or edit an existing one).
4. Enable **Lockdown Mode** for that role.
5. In the role's **App allowlist** section, choose which connectors (Salesforce, Slack, GitHub, etc.) are still permitted. Anything not allowlisted is automatically blocked.
6. Assign the role to the user(s) or group(s) that should be locked down by default.
...
1. **Cross-vendor portability.** A "lockdown" profile you can take from ChatGPT to Claude to Gemini to Copilot, with a portable threat-model manifest.
2. **Hardware-attested sessions.** The same way Apple's Private Cloud Compute refuses to run on non-attested servers, expect AI vendors to require a device-level attestation (TPM-bound) for the highest-security sessions.
3. **Audit-trail-grade logging by default.** The "what did my AI do and who did it talk to" log, exportable as a standard OpenTelemetry span, will become table stakes for any enterprise AI rollout.
...
1. **Open ChatGPT → Settings → Security → Lockdown Mode. Turn it on.** It takes 10 seconds.
2. **Open a new chat.** Paste the most sensitive document you have been working on this month. Ask the model to do whatever work you have been doing on it. Notice which features are missing — the workflow will feel different, but the work will still get done.
3. **When the work is done, flip Lockdown Mode off.** Return to your normal ChatGPT session for the rest of the evening.
4. **Tomorrow morning, when you are about to paste the next sensitive thing, do the same 10-second drill.**
